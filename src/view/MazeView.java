...
public final class MazeView extends JFrame {
    ...
    /**
     * Called by controller after every move. Receives rooms and player coordinates — view
     * never calls getPlayer().
     */
    ...
    public void showWin() {
        ...
        JOptionPane.showMessageDialog(this,
                "<html><h1 style='color:#00ff88'>🎉 CONGRATULATIONS! 🎉</h1><br>"
    ...
    private void updateButtons() {
        final boolean enabled = !myIndicator && !myEnding;
        myNorth.setEnabled(enabled);
        mySouth.setEnabled(enabled);
        myEast.setEnabled(enabled);
        myWest.setEnabled(enabled);
    }
    ...
    private final class MazePanel extends JPanel {
        ...
        protected void paintComponent(final Graphics theGraphics) {
            ...
            final int cell = Math.min((getWidth() - PADDING[3]) / cols,
                    (getHeight() - PADDING[3]) / rows);
            ...
            graphics2D.drawString("EXIT", someX + exitX * cell + cell / RATIOS[1],
                    someY + exitY * cell + cell / 2);

            for (int y = 0; y < rows; y++) {
                for (int x = 0; x < cols; x++) {
                    drawRoom(graphics2D, myRooms[y][x], someX + x * cell, someY + y * cell,
                            cell);
                }
            }
        ...
        private void drawRoom(final Graphics2D theGraphics, final TriviaRoom theRoom,
                final int theX, final int theY, final int theS) {
            ...
            if (theRoom != null) {
                ...
                if (theRoom.hasDoor(Direction.SOUTH)) {
                    drawDoor(theGraphics, theX + margin, theY + theS, theX + theS - margin,
                            theY + theS, theRoom.getDoor(Direction.SOUTH));
                }
                if (theRoom.hasDoor(Direction.EAST)) {
                    drawDoor(theGraphics, theX + theS, theY + margin, theX + theS,
                            theY + theS - margin, theRoom.getDoor(Direction.EAST));
                }
        ...
        private void drawDoor(final Graphics2D theGraphics, final int theFirstX,
                final int theFirstY, final int theSecondX, final int theSecondY,
    ...
    private final class QuestionPanel extends JPanel {
        ...
        private QuestionPanel() {
            ...
            setBorder(BorderFactory.createTitledBorder(
                    ...
                    " QUESTION AREA ", 0, 0, new Font(FONT, Font.BOLD, FONT_SIZES[1]),
                    Color.WHITE));
        ...
        public void loadQuestion(final AbstractTriviaQuestion theQ, final Direction theDir) {
            myDirection = theDir;
            myPrompt.setText("<html><div style='text-align:center;width:100%;max-width:400px'>"
                    + theQ.getQuestion() + "</div></html>");
        ...
        private void createShort() {
            ...
            textField.addActionListener(
                    e -> myController.submitAnswer(textField.getText().trim(), myDirection));
